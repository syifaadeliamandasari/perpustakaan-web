<x-app-layout>
    <section class="d-flex flex-column justify-content-center align-items-center text-center mt-5 py-5 px-4">
        <h1 class="mt-4 fs-2 fw-bold">Selamat Datang di StarLibrary <span style="color: green;">{{ auth()->user()->name }}</span>!</h1>
                ...
                <div class="container" style="display: flex;">
                    @foreach ($popularBooks as $popularBook)
                        <a href="{{ route('preview', $popularBook) }}" class="col text-dark text-decoration-none">
                            <div class="card" style="flex: 0 0 auto; max-width: 300px;">
                                ...
                                <div class="card-body text-center">
                                    ...
                                    <span class="fs-6">Dipinjam
                                        <span class="fw-bold text-decoration-underline">{{ $popularBook->borrows_count }}</span>
                                        kali</span>
                                </div>
                            </div>
                        </a>
                    ...
                    <a href="{{ route('preview', $newestBook) }}" class="col text-dark text-decoration-none">
                        <div class="card">
                            <img src="{{ isset($newestBook->cover) ? asset('storage/' . $newestBook->cover) : asset('storage/placeholder.png') }}"
                                ...
                                <div class="card-body text-center">
                                    <h3 class="card-text fs-5 fw-bold mb-3">{{ $newestBook->title }}</h3>
                                
                                    <div class="mb-3">
                                        <div class="ms-3 px-3 py-1 text-white bg-primary rounded-5" style="display: inline-block;">
                                            {{ $newestBook->category }}
                                        </div>
                                    </div>

                                    <div class="mb-3">
                                        <div class="ms-3 px-3 py-1 text-white bg-success rounded-5" style="display: inline-block;">
                                            {{ $newestBook->publisher }}
                                        </div>
                                    </div>
                                
                                    <span class="fs-6">Terbit
                                        <span class="fw-bold text-decoration-underline">{{ $newestBook->created_at->locale('id_ID')->diffForHumans() }}</span>
                                    </span>
                                </div>                                
                        </div>
                    </a>
